...
function get_user_order_goods($user_id, $num = 10, $start = 0)
{
    /* 取得订单列表 */
    $arr    = array();

    $sql = "SELECT rec_id,order_id, order_sn, goods_name,single_price, goods_number,single_price*goods_number pay_total ,FROM_UNIXTIME(apply_for_time), pay_another_status " .
        " FROM " .$GLOBALS['ecs']->table('order_goods') .
        " WHERE user_id = '$user_id' ORDER BY order_time DESC ";
    $res = $GLOBALS['db']->SelectLimit($sql, $num, $start);
